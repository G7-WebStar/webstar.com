...
// Check if there is at least one file with a non-empty attachment
$hasFiles = false;
...
<div class="d-flex flex-column flex-nowrap overflow-x-hidden">
    <?php while ($file = mysqli_fetch_assoc($fileResult)): ?>
        ...
        <?php
        $fileName = $file['fileTitle'];          // Display title
...
<!-- Toast Container -->
<div id="toastContainerFiles" class="position-absolute top-0 start-50 translate-middle-x pt-5 pt-md-1 d-flex flex-column align-items-center"
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('a[download]').forEach(link => {
        link.addEventListener('click', () => showToast('File successfully downloaded!'));
    });
});
...
function openTodoViewer(fileName, filePath) {
    ...
    viewer.innerHTML = "";
    const ext = filePath.split(".").pop().toLowerCase(); // <-- use filePath here!
